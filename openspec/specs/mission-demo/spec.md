# mission-demo Specification

## Purpose
The select screen's demo pane: each mission's compact `DemoScript`, fed one
key at a time through a real engine into a throwaway `Sim`, lit keycaps, the
loop, and the test that keeps every demo honest. Presentation only; the live

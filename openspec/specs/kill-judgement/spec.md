# kill-judgement Specification

## Purpose
One module, under `src/sim`, that decides what a kill cost against what it
could have cost: the field as it stood at the anchor command, the
keystrokes spent since, and the oracle's cheapest answer. The Motion
Ledger, the PERFECT medal and drill scoring all read it, so they cannot
disagree. The oracle it calls knows search, counted operators and the
paragraph motions, and ranks a search or a count half a keystroke dearer
than a plain answer of the same length so a tie never displaces a verdict.

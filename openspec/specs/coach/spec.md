...
## Purpose
Which drill the player needs, read off the lifetime ledger. Every kill the sim
judges leaves a `missed` count against each motion that would have been
optimal and was never pressed; the coach turns those counts into a ranking
of drill families and says it in three places - the drills screen, the
service record and the death screen - so the verdict is never buried again.
Pure over the save's `lifetime` shape and it lives in `src/sim`, so the
browser and the headless harness rank identically.

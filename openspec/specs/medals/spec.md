...
## Purpose
What earns a medal, when it is judged, and what it pays. The game already
knows, after every kill, whether you played it optimally and how many zombies
one command took down; medals are those two facts said out loud. Judging is
pure and lives in `src/sim`, so the browser, the headless harness, the smoke
bot and the replay checker cannot disagree about what you earned.

Feeds `run-wallet` (the payout) and `salvage-earning` (the lifetime credit);
drawn by `hud-chrome`.
## Requirements

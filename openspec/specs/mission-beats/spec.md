# mission-beats Specification

## Purpose
What happens inside one mission: TRY (the scene, with the warm-up rules) and
DONE (keystrokes against par, stars, and the keys that leave it), the par the
oracle computes for each scene, the star thresholds, the mission strip and the
`MISSION` line in `text()`. The demo that used to be a WATCH beat lives on the
select screen instead (`mission-demo`).
## Requirements
